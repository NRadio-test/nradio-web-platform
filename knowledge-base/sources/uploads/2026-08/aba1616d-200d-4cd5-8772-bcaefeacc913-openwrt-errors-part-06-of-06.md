...
容器数据目录若位于不支持 overlay2 特性的文件系统、TF 普通小 overlay 或只读挂载，会启动失败。把 Docker root 移到可靠的 ext4/btrfs 外置盘，确认 inode、空间、内核模块和 mount propagation；OpenWrt 自身 overlay 与 Docker overlay2 是不同层。容器网络还会增加 bridge、nft 和 MTU复杂度。
